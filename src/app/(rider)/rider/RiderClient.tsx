...
import {
  ...
  markDeliveredAction,
} from "@/features/staff/actions/staffOrderActions";
import {
  MapPin,
  Box,
  ArrowRight,
...
function formatAddress(addr: RiderOrder["addresses"]) {
  if (!addr) return "—";
  const parts = [addr.building, addr.street, addr.area, addr.city].filter(
    Boolean,
  );
...
export function RiderClient({ initialOrders, riderName }: RiderClientProps) {
  ...
  const activeDelivery =
    orders.find((o) => o.status === "out_for_delivery") ?? null;
    ...
    <div className="space-y-5 max-w-lg mx-auto">
      ...
      >
        <div className="flex items-center gap-3">
          <p className="font-bold text-sm tracking-wide">Hey, {riderName}!</p>
          {isOnline && (
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          {
            label: "Earnings",
            value: `₹${recent.reduce((s, o) => s + o.total, 0).toFixed(0)}`,
            icon: IndianRupee,
