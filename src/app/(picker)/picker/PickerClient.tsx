...
interface PickerOrder {
    ...
    addresses: { building: string | null; street: string | null; area: string | null; city: string | null } | null;
...
export function PickerClient({ initialOrders, pickerName }: PickerClientProps) {
    ...
    // ── Derived state ──
    const queue = orders.filter((o) => ["processing", "confirmed"].includes(o.status));
        ...
        <div className="space-y-5 max-w-lg mx-auto">
            ...
            <div className="flex items-center justify-between">
                <p className="text-sm text-muted-foreground font-medium">
                    Hey, <span className="text-foreground font-semibold">{pickerName}</span>!
                </p>
            ...
            <div className="grid grid-cols-3 gap-3">
                {[
                    { label: "Pending", value: String(pendingCount), icon: Clock, color: "bg-amber-500/10 text-amber-600" },
                    { label: "Picked Today", value: String(todayPicked), icon: Package, color: "bg-emerald-500/10 text-emerald-600" },
                    { label: "Active", value: activeOrderId ? "1" : "0", icon: Timer, color: "bg-blue-500/10 text-blue-600" },
                ].map((s) => (
                    <div
                        key={s.label}
                    ...
                    <div className="bg-card border border-primary/20 rounded-2xl shadow-md overflow-hidden">
                        ...
                        <div className="px-4 pt-4 pb-3 flex justify-between items-start">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground mt-0.5">
                                    ...
                                    {activeOrder.addresses?.area || activeOrder.addresses?.city || "—"}
                                </p>
                            </div>
                            <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full flex items-center gap-1 ${isUrgent(activeOrder.created_at)
                        ...
                        <div className="px-4 pb-3 space-y-1 max-h-56 overflow-y-auto">
                            {activeOrder.order_items.map((item) => {
                                const name = (item.product_snapshot as { name?: string } | null)?.name || "Unknown Item";
                                const unit = (item.product_snapshot as { unit?: string } | null)?.unit || "";
                                return (
                                    <div
                                        key={item.id}
                                        className="flex items-center gap-3 py-2 px-3 rounded-xl bg-muted/30 hover:bg-muted/50 transition-colors"
                                    >
                        ...
                        {queue
                            ...
                            .map((order) => {
                                ...
                                return (
                                    <div
                                        key={order.id}
                                        className={`bg-card border rounded-2xl shadow-sm p-4 hover:shadow-md transition-shadow ${urgent ? "border-red-200 dark:border-red-500/30" : "border-border/50"
                                            }`}
                                    >
                                        <div className="flex justify-between items-start mb-3">
                                            ...
                                            <span
                                                className={`text-[10px] font-bold px-2 py-1 rounded-full flex items-center gap-1 ${urgent ? "bg-red-500/10 text-red-500" : "bg-amber-500/10 text-amber-500"
                                                    }`}
                                            >
                                                <Clock className="w-3 h-3" />
                                        ...
                                        <div className="flex items-center gap-2 mb-3">
                                            ...
                                            <span className="text-xs text-muted-foreground truncate">
                                                {order.order_items
                                                    .slice(0, 3)
                                                    .map((i) => (i.product_snapshot as { name?: string } | null)?.name || "Item")
                                                    .join(", ")}
            ...
            <section>
                ...
                {showCompleted && (
                    <div className="bg-card rounded-2xl border border-border/50 shadow-sm overflow-hidden divide-y divide-border/50">
                        {completed.length === 0 ? (
                            <p className="p-4 text-sm text-muted-foreground text-center">None yet today</p>
                        ) : (
                            completed.map((o) => (
                                <div key={o.id} className="px-4 py-3 flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-xs text-muted-foreground mt-0.5">
                                            {o.profiles?.name || "Customer"} • {o.order_items.length} items
                                        </p>
                                    </div>
