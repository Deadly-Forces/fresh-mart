...
export default function FAQPage() {
  const faqs = [
    ...
    },
    {
      ...
      a: "Free delivery applies to all orders over ₹499. For orders under ₹499, a standard delivery fee of ₹49 applies.",
    },
    {
      ...
      <div className="container mx-auto px-4 max-w-4xl py-16 lg:py-20">
        <div className="space-y-4">
          {faqs.map((faq, i) => (
            <div
              key={i}
              className="group bg-card border border-border/50 rounded-2xl p-6 sm:p-8 hover:border-primary/20 hover:shadow-soft transition-all duration-300"
            >
        ...
        <div className="mt-14 text-center p-8 rounded-3xl bg-gradient-to-r from-primary/5 via-emerald-500/5 to-teal-500/5 border border-primary/10">
          ...
          <a
            href="/contact"
            className="inline-flex h-11 px-6 items-center gap-2 rounded-xl bg-gradient-to-r from-primary to-emerald-500 text-white font-medium hover:shadow-glow transition-all duration-300 text-sm"
          >
            Contact Support
          </a>
        </div>
