...
export default function CartPage() {
    const { items, removeItem, updateQuantity, getTotal, clearCart, appliedPromo, applyPromo, removePromo } = useCartStore();
        ...
        return (
            <div className="container mx-auto px-4 max-w-7xl py-24 text-center flex flex-col items-center justify-center min-h-[60vh]">
                ...
                <p className="text-muted-foreground text-sm mb-8 max-w-sm">
                    Looks like you haven&apos;t added anything yet. Start exploring our fresh collection.
                </p>
                <Link href="/shop">
                    <Button size="lg" className="h-12 px-8 rounded-xl font-medium text-sm bg-gradient-to-r from-primary to-emerald-500 hover:shadow-glow transition-all duration-300">
                        Start Shopping
                    </Button>
                </Link>
            </div>
    ...
    return (
        <div className="container mx-auto px-4 max-w-7xl py-8 md:py-12">
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-2xl font-bold text-foreground tracking-tight">Shopping Cart</h1>
                    <p className="text-sm text-muted-foreground mt-1">{items.length} item{items.length !== 1 ? "s" : ""} in your cart</p>
                </div>
                <Button variant="ghost" className="text-destructive gap-2 text-sm hover:bg-destructive/10 rounded-xl px-3 h-9" onClick={clearCart}>
            ...
            <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="flex-1 space-y-3">
                    {items.map((item) => (
                        <div key={item.id} className="flex gap-4 p-4 rounded-2xl bg-card border border-border/40 hover:border-primary/15 hover:shadow-soft transition-all duration-300">
                        ...
                        <div className="relative bg-card border border-border/50 rounded-2xl p-6 shadow-aesthetic">
                            ...
                            <div className="space-y-3 text-sm">
                                ...
                                {appliedPromo && (
                                    <div className="flex justify-between items-center text-emerald-600 font-medium pt-2 border-t border-border/30">
                                        <span className="flex items-center gap-2">Discount ({appliedPromo.code})</span>
                                ...
                                {!appliedPromo ? (
                                    <div className="flex gap-2">
                                        ...
                                        <Button
                                            ...
                                            onClick={async () => {
                                                ...
                                                try {
                                                    const res = await validatePromoCodeAction(promoInput, subtotal);
                                                    if (res.error) {
                                                        toast.error(res.error);
                                                    } else if (res.success && res.promo) {
                                                        applyPromo(res.promo);
                                                        setPromoInput("");
                                                        toast.success(`Promo code applied! Saved $${res.promo.discountAmount}`);
                                                    }
                                                } finally {
                                    ...
                                    <div className="flex items-center justify-between p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs text-emerald-600/80 font-medium">Applied</span>
                                        </div>
                                        <Button
                                            variant="ghost"
                                            size="icon"
                            ...
                            {subtotal < 50 && (
                                <div className="bg-gradient-to-r from-primary/5 to-emerald-500/5 rounded-xl p-3.5 mb-5 border border-primary/10">
                                    <p className="text-xs text-muted-foreground mb-2 text-center">
                                        Add <span className="font-semibold text-primary">${(50 - subtotal).toFixed(2)}</span> more for free delivery
                                    </p>
                                    <div className="h-2 bg-secondary rounded-full overflow-hidden">
                                        <div
                                            className="h-full bg-gradient-to-r from-primary to-emerald-500 rounded-full transition-all duration-500"
                                            ref={(el) => { if (el) el.style.width = `${Math.min((subtotal / 50) * 100, 100)}%`; }}
                                        />
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="border-t border-border/50 pt-5">
                                <div className="flex justify-between items-center mb-5">
                                    ...
                                    <span className="text-2xl font-bold bg-gradient-to-r from-primary to-emerald-500 bg-clip-text text-transparent">${total.toFixed(2)}</span>
                                </div>
                                <Link href="/checkout">
