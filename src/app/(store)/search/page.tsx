                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            <h1 className="text-3xl md:text-4xl font-bold text-foreground tracking-tight mb-1">
                                {query ? (
                                    <>
                                        Results for &ldquo;<span className="bg-gradient-to-r from-primary to-emerald-500 bg-clip-text text-transparent">{query}</span>&rdquo;
                                    </>
                                ) : (
                                    <>Search <span className="bg-gradient-to-r from-blue-500 to-cyan-500 bg-clip-text text-transparent">Products</span></>
                                )}
                            </h1>
                            {query && (
                            ...
                            {!query && (
                                <p className="text-muted-foreground">Find exactly what you need from our fresh collection</p>
                            )}
                        </div>
                    </div>
                </div>
                        ...
                        {query && (
                            <div className="flex items-center justify-between mb-6 bg-background/70 backdrop-blur-sm rounded-xl border border-border/50 px-4 py-3">
                                <p className="text-sm text-muted-foreground">
                                    <span className="font-semibold bg-gradient-to-r from-primary to-emerald-500 bg-clip-text text-transparent">12</span> results
                                </p>
                                <SortDropdown />
                            </div>
                        )}
                        ...
                        {query ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-20 text-center">
                                ...
                                <p className="text-muted-foreground max-w-md mb-8">
                                    Type a keyword to search for groceries, fresh produce, and daily
                                    essentials.
                                </p>
                                <div className="flex flex-wrap justify-center gap-2">
                                    {[
                                        { name: "Vegetables", color: "from-emerald-500/10 to-green-500/10 border-emerald-500/20 hover:border-emerald-500/40 text-emerald-700 dark:text-emerald-400" },
                                        { name: "Fruits", color: "from-orange-500/10 to-amber-500/10 border-orange-500/20 hover:border-orange-500/40 text-orange-700 dark:text-orange-400" },
                                        { name: "Dairy", color: "from-blue-500/10 to-cyan-500/10 border-blue-500/20 hover:border-blue-500/40 text-blue-700 dark:text-blue-400" },
                                        { name: "Snacks", color: "from-purple-500/10 to-pink-500/10 border-purple-500/20 hover:border-purple-500/40 text-purple-700 dark:text-purple-400" },
                                    ].map((cat) => (
                                        <Link
                                            key={cat.name}
