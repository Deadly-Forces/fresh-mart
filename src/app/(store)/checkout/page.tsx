...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    if (invalidItems.length > 0) {
      ...
      toast.error(
        `Removed ${invalidItems.length} invalid item(s) from cart. Please try again.`,
      );
      return;
    }
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center justify-between mt-10 pt-6 border-t border-border/40">
            ...
            {currentStep < 2 ? (
              <Button
                onClick={() => setCurrentStep(currentStep + 1)}
                className="gap-2 px-6 rounded-xl h-11 bg-gradient-to-r from-primary to-emerald-500 hover:shadow-glow transition-all duration-300"
                disabled={isPending}
              >
