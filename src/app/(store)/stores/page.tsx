        ...
        <div className="w-full lg:w-1/3 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-4 overflow-y-auto pr-2 max-h-[500px]">
            {stores.map((store, i) => (
              <div
                key={i}
                className="bg-background/70 backdrop-blur-sm border border-border/50 rounded-2xl p-5 hover:border-primary/30 hover:shadow-card-hover transition-all duration-300 cursor-pointer group"
              >
                <div className="flex items-start justify-between mb-2">
                  <h3 className="font-bold text-lg group-hover:text-primary transition-colors">
                    {store.name}
                  </h3>
                  <span className="text-xs font-bold text-emerald-600 bg-gradient-to-r from-emerald-500/10 to-green-500/10 border border-emerald-500/20 px-2.5 py-1 rounded-full">
                    {store.status}
                  </span>
                </div>
        ...
        <div className="w-full lg:w-2/3 h-[400px] lg:h-auto min-h-[500px] bg-gradient-to-br from-secondary/30 to-secondary/10 rounded-3xl border border-border/50 flex items-center justify-center relative overflow-hidden shadow-soft">
          ...
          <div className="relative z-10 text-center flex flex-col items-center">
            ...
            <p className="text-sm text-muted-foreground">
              Map rendering component placeholder
            </p>
          </div>
        </div>
      </div>
