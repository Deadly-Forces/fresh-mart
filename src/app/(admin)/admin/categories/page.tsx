        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="bg-card border border-border rounded-card p-4 text-center">
                    ...
                    <p className="text-xs text-muted-foreground">Total Products Across Categories</p>
                </div>
            </div>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="bg-secondary/50 border-b border-border">
                                ...
                                <th className="px-4 py-3 text-left font-semibold text-muted-foreground text-xs">Image</th>
                                <th className="px-4 py-3 text-left font-semibold text-muted-foreground text-xs">Name</th>
                                <th className="px-4 py-3 text-left font-semibold text-muted-foreground text-xs">Slug</th>
                                <th className="px-4 py-3 text-left font-semibold text-muted-foreground text-xs">Products</th>
                        ...
                        <tbody className="divide-y divide-border">
                            {categories.length === 0 ? (
                                <tr>
                                    <td colSpan={7} className="px-4 py-8 text-center text-muted-foreground">No categories found.</td>
                                </tr>
                                ...
                                <tr key={c.id} className="hover:bg-secondary/30 transition-colors">
                                    ...
                                    <td className="px-4 py-3">
                                        {c.image_url ? (
                                            <img src={c.image_url} alt={c.name} className="w-10 h-10 object-cover rounded-md border" />
                                        ) : (
                                    ...
                                    <td className="px-4 py-3">
                                        ...
                                        <span className="text-xs">{c.active ? "Active" : "Hidden"}</span>
                                    </td>
                                    <td className="px-4 py-3 text-right">
                                        <div className="flex justify-end gap-2">
                                            <ToggleCategoryButton categoryId={c.id} isActive={c.active} />
