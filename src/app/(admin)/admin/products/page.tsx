        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <div className="flex gap-2">
                    <Button variant="outline" size="sm" className="gap-1"><Upload className="w-4 h-4" /> Import</Button>
                    <Button variant="outline" size="sm" className="gap-1"><Download className="w-4 h-4" /> Export</Button>
                    <Link href="/admin/products/new">
                        <Button size="sm" className="gap-1"><Plus className="w-4 h-4" /> Add Product</Button>
                    </Link>
                </div>
            </div>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="bg-secondary/50 border-b border-border">
                                ...
                                <th className="px-4 py-3 text-left font-semibold text-muted-foreground text-xs">Product</th>
                                <th className="px-4 py-3 text-left font-semibold text-muted-foreground text-xs">Category</th>
                                <th className="px-4 py-3 text-left font-semibold text-muted-foreground text-xs">Price</th>
                                <th className="px-4 py-3 text-left font-semibold text-muted-foreground text-xs">Stock</th>
                                ...
                                <tr key={p.id} className="hover:bg-secondary/30 transition-colors">
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="flex items-center gap-3">
                                            {p.image_url ? (
                                                <img src={p.image_url} alt={p.name} className="w-10 h-10 object-cover rounded-md shrink-0 border" />
                                            ) : (
                                    ...
                                    <td className="px-4 py-3">
                                        <InlineStockEditor productId={p.id} currentStock={p.stock} />
                                    </td>
                                    <td className="px-4 py-3">
                                        <span className={`w-2 h-2 rounded-full inline-block mr-1.5 ${p.active ? "bg-success" : "bg-muted"}`} />
                                        <span className="text-xs">{p.active ? "Active" : "Draft"}</span>
                                    </td>
