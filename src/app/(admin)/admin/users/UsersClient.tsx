  ...
  const handleRoleChange = (userId: string, newRole: string) => {
    ...
    startTransition(async () => {
      const result = await updateUserRoleAction(
        userId,
        newRole as "customer" | "admin" | "delivery",
      );
      setUpdatingId(null);
      if (result.error) {
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <select
                        ...
                        className={`px-2 py-1 text-xs rounded-md border border-border bg-background capitalize cursor-pointer ${
                          isPending && updatingId === u.id ? "opacity-50" : ""
                        }`}
                      >
