...
export default async function AdminUsersPage() {
    ...
    const users = allProfiles.map((p: any) => {
        ...
        const joinedDate = p.created_at ? new Date(p.created_at).toISOString().split("T")[0] : "N/A";
        return {
            id: p.id,
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="bg-card border border-border rounded-card p-4 text-center">
                    <p className="text-2xl font-bold">{users.filter((u) => u.orders > 0).length}</p>
                    <p className="text-xs text-muted-foreground">Active Customers</p>
                </div>
                <div className="bg-card border border-border rounded-card p-4 text-center">
                    <p className="text-2xl font-bold">${users.reduce((s, u) => s + u.spent, 0).toFixed(2)}</p>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="relative w-full sm:w-80">
                    ...
                    <Input placeholder="Search users by email or name..." className="pl-9 h-10" />
                </div>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="bg-secondary/50 border-b border-border">
                                ...
                                <th className="px-4 py-3 text-left font-semibold text-muted-foreground text-xs">Orders</th>
                                <th className="px-4 py-3 text-left font-semibold text-muted-foreground text-xs">Total Spent</th>
                        ...
                        <tbody className="divide-y divide-border">
                            {users.length === 0 ? (
                                ...
                                <tr key={u.id} className="hover:bg-secondary/30 transition-colors">
                                    ...
                                    <td className="px-4 py-3 font-semibold">${u.spent.toFixed(2)}</td>
                                    <td className="px-4 py-3 text-muted-foreground">{u.joined}</td>
                                    <td className="px-4 py-3">
                                        <span className={`px-2 py-0.5 text-xs rounded-pill font-medium capitalize ${u.role === "admin" ? "bg-primary/10 text-primary" : "bg-success/10 text-success"}`}>
                                            {u.role}
                                        </span>
                                    </td>
                                </tr>
