...
export default async function AdminAnalyticsPage() {
  ...
  const avgOrderValue =
    totalOrdersCount > 0 ? totalRevenue / (deliveredOrders.length || 1) : 0;
  ...
  orders.forEach((o: any) => {
    if (!o.user_id) return;
    if (!userOrderStats[o.user_id])
      userOrderStats[o.user_id] = { spent: 0, count: 0 };
  ...
  const revenueByDay = last7Days.map((day) => {
    const dayOrders = deliveredOrders.filter((o: any) =>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-3">
            ...
            {[
              ...
              const pct =
                totalOrdersCount > 0
                  ? ((count / totalOrdersCount) * 100).toFixed(1)
                  : "0";
              return (
                <div
                  key={status}
