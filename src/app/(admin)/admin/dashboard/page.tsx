import { TrendingUp, TrendingDown, ArrowUpRight, DollarSign, ShoppingCart, Users, AlertTriangle } from "lucide-react";
...
export default async function AdminDashboardPage() {
    ...
    const pendingOrdersCount = orders.filter((o: any) => o.status === "processing").length;
    ...
    const lastWeekOrders = orders.filter((o: any) => o.created_at >= fourteenDaysAgo && o.created_at < sevenDaysAgo);
    ...
    const kpis = [
        { label: "Total Revenue", value: `$${totalRevenue.toFixed(2)}`, change: `${Number(revenueChange) >= 0 ? "+" : ""}${revenueChange}%`, up: Number(revenueChange) >= 0, icon: DollarSign },
        { label: "Total Orders", value: totalOrdersCount.toString(), change: `${Number(ordersChange) >= 0 ? "+" : ""}${ordersChange}%`, up: Number(ordersChange) >= 0, icon: ShoppingCart },
        { label: "Total Customers", value: (customersCount || 0).toString(), change: `${customersCount || 0} registered`, up: true, icon: Users },
        { label: "Pending Orders", value: pendingOrdersCount.toString(), change: pendingOrdersCount > 0 ? "Needs Action" : "All Clear", up: pendingOrdersCount === 0, icon: AlertTriangle },
    ];
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-end mb-2">
                <AutoRefresh intervalMs={30000} tables={["orders", "products", "profiles"]} />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                {kpis.map((kpi) => {
                    const Icon = kpi.icon;
                    return (
                        <div key={kpi.label} className="bg-card border border-border rounded-card p-5 shadow-sm">
                            <div className="flex items-start justify-between mb-4">
                                <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary">
                                    <Icon className="w-5 h-5" />
                                </div>
                                <span className={`flex items-center gap-0.5 text-xs font-semibold ${kpi.up ? "text-success" : "text-destructive"}`}>
                ...
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex gap-1">
                        {["Daily", "Weekly", "Monthly"].map((tab, i) => (
                            <button
                                key={tab}
                                className={`px-3 py-1 rounded-button text-xs font-medium ${i === 1 ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-secondary"
                                    }`}
                            >
                                {tab}
                            </button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-card border border-border rounded-card p-6 shadow-sm">
                    ...
                    {recentOrders.length === 0 ? (
                        <p className="text-sm text-muted-foreground italic py-4">No recent orders found.</p>
                    ) : (
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-border">
                                        <th className="py-2 text-left font-medium text-muted-foreground text-xs">Order ID</th>
                                        <th className="py-2 text-left font-medium text-muted-foreground text-xs">Customer</th>
                                        <th className="py-2 text-left font-medium text-muted-foreground text-xs">Total</th>
                                        <th className="py-2 text-left font-medium text-muted-foreground text-xs">Status</th>
                                    </tr>
                                </thead>
                ...
                <div className="bg-card border border-border rounded-card p-6 shadow-sm">
                    ...
                    {lowStock.length === 0 ? (
                        <p className="text-sm text-muted-foreground italic py-4">All products are well stocked.</p>
                    ) : (
                        <div className="space-y-3">
                            {lowStock.map((item) => (
                                <div key={item.name} className="flex items-center justify-between p-3 bg-destructive/5 rounded-button border border-destructive/10">
                                    <div>
