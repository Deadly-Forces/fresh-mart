...
const INITIAL_STATE: OnboardingState = {
    ...
    countryCode: "+91",
    phone: "",
    address: { building: "", street: "", area: "", landmark: "", city: "", state: "", pincode: "" },
    dietaryPref: "veg",
...
export default function OnboardingPage() {
        ...
        if (saved) {
            ...
            supabase.auth.getUser().then(({ data: { user } }) => {
                if (user?.user_metadata?.full_name) {
                    setState((s) => ({ ...s, name: user.user_metadata.full_name, avatarUrl: user.user_metadata.avatar_url || "" }));
                }
            });
        }
    }, [supabase.auth]);
    ...
    const updateAddress = (field: keyof OnboardingState["address"], value: string) => {
        setState((prev) => ({
            ...prev,
    ...
    const handleNext = async () => {
        ...
        try {
            ...
            console.error("Error saving onboarding:", error, JSON.stringify(error, Object.getOwnPropertyNames(error)));
            alert("Failed to save onboarding: " + (error?.message || JSON.stringify(error)));
            ...
            <div className="bg-card w-full max-w-[560px] p-6 sm:p-10 rounded-card shadow-modal">
                ...
                <div className="flex items-center justify-between mb-10 relative">
                    ...
                    {[1, 2, 3].map((step) => (
                        <div
                            key={step}
                            className={`w-10 h-10 rounded-full flex items-center justify-center z-10 transition-colors ${state.step >= step ? "bg-success text-white" : "bg-muted text-muted-foreground border-2 border-border"
                                }`}
                        >
                            {step}
                        </div>
                ...
                <div className="min-h-[320px]">
                    ...
                    {state.step === 2 && (
                        <OnboardingAddressStep state={state} onUpdate={updateState} onUpdateAddress={updateAddress} />
                    )}
                    {state.step === 3 && (
                ...
                <div className="flex items-center justify-between mt-8 pt-6 border-t border-border">
                    ...
                    <Button
                        onClick={handleNext}
                        disabled={isLoading || (state.step === 1 && !state.name) || (state.step === 2 && (!state.phone || !state.address.street))}
                        className="px-8 min-w-[140px]"
                    >
                        {isLoading ? "Saving..." : state.step === 3 ? "Complete Setup" : "Continue"}
                    </Button>
                </div>
            </div>
        </div>
