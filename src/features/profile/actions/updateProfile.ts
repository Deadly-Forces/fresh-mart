...
const profileSchema = z.object({
    ...
    phone: z.string().max(20).regex(/^[\d+\-() ]*$/, "Invalid phone number format").nullable(),
...
const addressFieldSchema = z.object({
    ...
    pincode: z.string().regex(/^(\d{6})?$/, "Pincode must be 6 digits").nullable(),
});
...
export async function updateProfileAction(formData: FormData) {
    ...
    if (!addressValidation.success) {
        const firstError = addressValidation.error.issues[0]?.message || "Invalid address data.";
    ...
    if (!profileValidation.success) {
        const firstError = profileValidation.error.issues[0]?.message || "Invalid profile data.";
        return { error: firstError };
    }
    ...
    const profileUpdates = {
        ...
        dietary_preference: (profileValidation.data.dietary_pref || null) as DietaryPref | null,
