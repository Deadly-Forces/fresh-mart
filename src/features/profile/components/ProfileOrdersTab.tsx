...
const OrderTrackingMap = dynamic(
  () =>
    import("@/features/profile/components/OrderTrackingMap").then(
      (m) => m.OrderTrackingMap,
    ),
  { ssr: false },
);
          ...
          setLocalOrders((prev) =>
            prev.map((o) =>
              o.id === updatedOrder.id
                ? {
                    ...o,
                    status: updatedOrder.status as UserOrder["status"],
                    payment_status: updatedOrder.payment_status ?? undefined,
                  }
                : o,
            ),
          );
    ...
    return (
      <div className="bg-card border border-border rounded-2xl p-16 text-center shadow-sm">
        ...
        <p className="text-muted-foreground mt-2 max-w-sm mx-auto">
          Start shopping to see your order history here. We&apos;ve got fresh
        ...
        {localOrders.map((order) => (
          ...
          >
            ...
            {(() => {
              ...
              const isSimulatedDelivered =
                progressPercent >= 100 || order.status === "delivered";
              const actualDeliveryMs = createdMs + deliveryWindowMs;
              const oneHourMs = 60 * 60 * 1000;
              const twoHoursMs = 2 * 60 * 60 * 1000;

              const canCancel =
                !isSimulatedDelivered &&
                ["pending", "processing", "confirmed"].includes(order.status) &&
                order.status !== "cancelled";
              const canReturn =
                isSimulatedDelivered &&
                order.status !== "cancelled" &&
                (now ? now - actualDeliveryMs < oneHourMs : false);
              const canReplaceOnly =
                isSimulatedDelivered &&
                order.status !== "cancelled" &&
                !canReturn &&
                (now ? now - actualDeliveryMs < twoHoursMs : false);
              ...
              return (
                <>
                  {/* Order status strip */}
                  <div
                    className={`h-1 transition-all duration-1000 ease-linear rounded-tl-2xl ${
                    ...
                    ref={(el) =>
                      el?.style.setProperty("--progress", `${progressPercent}%`)
                    }
                  />
                    ...
                    {order.status !== "cancelled" && (
                      <div className="mb-5">
                        <div className="flex items-center justify-between relative">
                          ...
                          <div
                            className="absolute top-3 left-[16px] h-0.5 bg-primary transition-all duration-1000 ease-linear w-progress-offset"
                            ref={(el) =>
                      ...
                      <div className="pt-4 border-t border-border/50 mt-4">
                        ...
                        <div className="space-y-2 mt-4 ml-8 md:ml-[60px] max-w-2xl bg-secondary/10 p-3 rounded-xl border border-secondary/20">
                          ...
                          {order.items.length > 3 && (
                            ...
                            >
                              <ChevronRight
                                className={`w-3 h-3 transition-transform ${expandedOrders[order.id] ? "rotate-90" : ""}`}
                              />{" "}
                              {expandedOrders[order.id]
                                ? "Show less"
                    ...
                    <div className="pt-4 border-t border-border/50 mt-4 flex justify-end gap-2 flex-wrap">
                      {isTrackable(displayStatus) && (
                        <Button
                          ...
                          variant="outline"
                          size="sm"
                          className={`gap-1.5 h-8 text-xs font-semibold rounded-full active:scale-95 transition-all ${
                      ...
                      {canReplaceOnly && (
                        <Button
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            router.push(
                              `/profile/returns/${order.id}?type=replace`,
                            );
                          }}
                        >
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete this order from your history? This
