...
export function ReturnRequestClient({
  ...
  const processAgenticAI = (
    text: string,
    hasImage: boolean,
    reqType: "return" | "replace",
  ) => {
    ...
    if (hasCriticalIssue && hasImage) {
      ...
      return {
        ...
        userMessage:
          "Our AI assistant noted your issue, but since no photo was provided, it must be manually reviewed before we can proceed.",
      };
    } else {
      return {
        status: "manual_review",
  ...
  const handleAnalyze = async (e?: React.FormEvent) => {
    ...
    if (!reason || reason.trim().length < 10) {
      toast.error(
        "Please provide a more detailed reason for the return/replacement.",
      );
      return;
    }
      ...
      <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
        <div className="p-6 border-b border-border bg-secondary/30">
          <h1 className="text-2xl font-heading font-bold">
            Initiate Return / Replace
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            Order #{order.id.slice(0, 8).toUpperCase()} • {order.items.length}{" "}
            items
          </p>
        </div>
        ...
        <form
          onSubmit={(e) => {
            e.preventDefault();
            if (!aiAnalysisResult) handleAnalyze();
          }}
        ...
        >
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <label
                className={`cursor-pointer flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all ${type === "return" ? "border-primary bg-primary/5" : "border-border hover:border-primary/50"}`}
              >
                <input
                  type="radio"
                  name="type"
                ...
                />
                <span
                  className={`font-bold ${type === "return" ? "text-primary" : "text-muted-foreground"}`}
                >
                  Refund to Wallet
                </span>
              ...
              <label
                className={`cursor-pointer flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all ${type === "replace" ? "border-primary bg-primary/5" : "border-border hover:border-primary/50"}`}
              >
                <input
                  type="radio"
                  name="type"
                ...
                />
                <span
                  className={`font-bold ${type === "replace" ? "text-primary" : "text-muted-foreground"}`}
                >
                  Replace Items
                </span>
                <span className="text-xs text-muted-foreground mt-1 text-center">
                  Get identical new items
                </span>
              </label>
            </div>
          </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-background border border-border text-xs font-semibold">
                      ...
                      <span
                        className={
                          aiAnalysisResult.status === "approved"
                            ? "text-green-600"
                            : "text-amber-500"
                        }
                      >
                        {aiAnalysisResult.status === "approved"
                          ? `Eligible for Auto-${type === "return" ? "Refund" : "Replacement"}`
