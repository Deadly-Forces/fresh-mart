...
import { MapPin, Plus, Loader2, Star, Edit2, Trash2, X, Save, Check } from "lucide-react";
...
export function AddressesSection() {
    ...
    const handleAddAddress = async (formData: FormData) => {
        ...
        if (res.error) {
            ...
            setAddresses([res.address, ...addresses.map(a => 
    ...
    const handleUpdateAddress = async (addressId: string, formData: FormData) => {
        ...
        if (res.error) {
            ...
            setAddresses(addresses.map(a => a.id === addressId ? res.address : a));
            setEditingId(null);
        }
        setIsSubmitting(false);
    };
            ...
            <div className="p-6 sm:p-7">
                ...
                {addresses.length === 0 && !isAddingMode && (
                    <div className="p-8 rounded-xl border-2 border-dashed border-border/60 text-center">
                        ...
                        <p className="text-muted-foreground italic mb-2">No addresses saved yet</p>
                        <Button
                            type="button"
                            variant="outline"
                            onClick={() => setIsAddingMode(true)}
                        ...
                        {addresses.map((addr) => (
                            <div key={addr.id}>
                                {editingId === addr.id ? (
                                    <AddressForm
                                        address={addr}
                                        onSubmit={(formData) => handleUpdateAddress(addr.id, formData)}
            ...
            <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                        ...
                        {address.is_default && (
                            <Badge variant="default" className="text-[10px] uppercase font-bold tracking-wider rounded-sm px-1.5 py-0 h-5 gap-1">
                                <Star className="w-3 h-3" /> Default
                            </Badge>
                        )}
                    </div>
                    {address.building && <p className="font-semibold text-base">{address.building}</p>}
                    {address.street && <p className="text-foreground">{address.street}</p>}
                    {address.area && <p className="text-foreground">{address.area}</p>}
                    {address.landmark && <p className="text-muted-foreground text-sm italic mt-1">Near {address.landmark}</p>}
                    <div className="flex items-center gap-2 mt-2 pt-2 border-t border-border/40">
                        <MapPin className="w-3.5 h-3.5 text-muted-foreground" />
                        <p className="text-muted-foreground text-sm">
                            {address.city}{address.city && address.state ? ", " : ""}{address.state}
                            {address.pincode && <span className="font-semibold text-foreground ml-2">PIN: {address.pincode}</span>}
                        </p>
                    </div>
                </div>
...
function AddressForm({ address, onSubmit, onCancel, isSubmitting, title }: AddressFormProps) {
    ...
    return (
        ...
        >
            <h4 className="font-semibold text-base border-b border-border/50 pb-2 mb-4">{title}</h4>
            <div className="space-y-3">
                <div className="space-y-1.5">
                    <Label htmlFor="label" className="text-xs font-semibold text-muted-foreground">Address Label</Label>
                    <Input
                        id="label"
                        name="label"
                ...
                <div className="space-y-1.5">
                    <Label htmlFor="street" className="text-xs font-semibold text-muted-foreground">Street / Road Name</Label>
                    <Input
                        id="street"
                        name="street"
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                        <Label htmlFor="area" className="text-xs font-semibold text-muted-foreground">Area / Locality</Label>
                        <Input
                            id="area"
                            name="area"
            ...
            <div className="flex gap-3 justify-end pt-4 mt-4 border-t border-border/50">
                ...
                <Button type="button" onClick={handleSubmit} disabled={isSubmitting} className="gap-2 min-w-[100px]">
