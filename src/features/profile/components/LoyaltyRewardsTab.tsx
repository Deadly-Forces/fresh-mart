...
export function LoyaltyRewardsTab() {
  ...
  const loadData = async () => {
    ...
    if (!result.error) {
      ...
      setTransactions((result.transactions as unknown as LoyaltyTransaction[]) ?? []);
    }
    setLoading(false);
  };
  ...
  const handleRedeem = async () => {
    ...
    if (result.error) {
      ...
      toast.success(
        `Redeemed ${pts} points for ₹${((result.discount as number) ?? 0).toFixed(2)} discount!`
      );
  ...
  const tier =
    totalEarned >= 5000
      ? { name: "Platinum", color: "from-slate-300 to-slate-500", textColor: "text-slate-300", next: null, progress: 100 }
      : totalEarned >= 2000
        ? { name: "Gold", color: "from-amber-400 to-yellow-600", textColor: "text-amber-400", next: "Platinum", progress: ((totalEarned - 2000) / 3000) * 100 }
        : totalEarned >= 500
          ? { name: "Silver", color: "from-gray-300 to-gray-500", textColor: "text-gray-300", next: "Gold", progress: ((totalEarned - 500) / 1500) * 100 }
          : { name: "Bronze", color: "from-orange-400 to-orange-600", textColor: "text-orange-400", next: "Silver", progress: (totalEarned / 500) * 100 };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {/* Balance Card */}
        <div className="sm:col-span-2 bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
          ...
          <div className="p-6 sm:p-8">
            <div className="flex items-start justify-between mb-6">
              ...
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-amber-500/10 to-yellow-500/10 flex items-center justify-center">
