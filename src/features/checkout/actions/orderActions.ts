...
import { rateLimit, isValidUUID, MAX_ORDER_TOTAL, MAX_ORDER_ITEMS, MAX_ITEM_QUANTITY } from "@/lib/security";
...
export async function placeOrderAction(
    ...
    try {
        ...
        if (!rateLimit(`order:${user.id}`, 5, 60_000)) {
            return { error: "Too many order attempts. Please wait a moment and try again." };
        }
        ...
        if (items.length > MAX_ORDER_ITEMS) {
            return { error: `Cannot place an order with more than ${MAX_ORDER_ITEMS} items.` };
        }
        ...
        const priceMap = new Map(dbProducts.map(p => [p.id, { ...p, price: Number(p.price) }]));
        for (const item of items) {
            ...
            if (!dbProduct.is_active) {
                return { error: `Product "${dbProduct.name}" is currently unavailable.` };
            }
            // Allow small floating-point tolerance (1 cent)
            if (Math.abs(item.price - dbProduct.price) > 0.01) {
                console.error(`Price mismatch for ${item.name}: cart=${item.price}, db=${dbProduct.price}`);
                return { error: `Price for "${dbProduct.name}" has changed. Please refresh your cart.` };
            }
            ...
            if ((dbProduct.stock ?? 0) < item.quantity) {
                ...
                return { error: `Only ${available} units of "${dbProduct.name}" available. Please reduce quantity.` };
            }
        }
        ...
        // Allow tolerance for: discount + delivery fee ($4.99) + express fee ($3.99) + rounding
        const maxFees = 4.99 + 3.99 + 1; // ~$10 tolerance for fees
        ...
        if (total < minExpectedTotal - 1 || total > maxExpectedTotal + 1) {
            console.error(`Total validation failed: client=${total}, subtotal=${calculatedSubtotal}, discount=${discountAmount}`);
            return { error: "Order total mismatch. Please refresh your cart and try again." };
        }
        ...
        const { data: rpcResult, error: rpcError } = await (supabase.rpc as any)('create_order_with_items', {
        ...
        const result = rpcResult as unknown as { success?: boolean; order_id?: string; error?: string };
        if (result.error) {
            console.error("Order creation failed:", result.error);
            if (result.error.includes("policy") || result.error.includes("permission")) {
                return { error: "Permission denied. Please try again or contact support." };
            }
            if (result.error.includes("violates foreign key")) {
                return { error: "One or more products are no longer available. Please refresh your cart." };
            }
        ...
        sendOrderConfirmationEmail(orderId, customerEmail, verifiedTotal).catch((e) => {
