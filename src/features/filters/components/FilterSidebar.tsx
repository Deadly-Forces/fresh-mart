...
/* ── Main component ── */
export function FilterSidebar({ className, categoryCounts = {} }: { className?: string; categoryCounts?: Record<string, number> }) {
    const router = useRouter();
    const pathname = usePathname();
    const searchParams = useSearchParams();

    // Local state for smooth dragging
    const initialPriceMax = Number(searchParams.get("maxPrice") || "22");
    const [localPrice, setLocalPrice] = useState(initialPriceMax);

    // Sync local state when URL changes externally
    React.useEffect(() => {
        setLocalPrice(Number(searchParams.get("maxPrice") || "22"));
    }, [searchParams]);
    ...
    const filterGroups: FilterGroup[] = React.useMemo(() => [
        {
            ...
            paramKey: "category",
            options: [
                { label: "Vegetables", value: "vegetables", count: categoryCounts["vegetables"] ?? 0 },
                { label: "Fruits", value: "fruits", count: categoryCounts["fruits"] ?? 0 },
                { label: "Dairy & Eggs", value: "dairy-eggs", count: categoryCounts["dairy-eggs"] ?? 0 },
                { label: "Bakery", value: "bakery", count: categoryCounts["bakery"] ?? 0 },
                { label: "Meat & Seafood", value: "meat-seafood", count: categoryCounts["meat-seafood"] ?? 0 },
                { label: "Snacks", value: "snacks", count: categoryCounts["snacks"] ?? 0 },
                { label: "Beverages", value: "beverages", count: categoryCounts["beverages"] ?? 0 },
                { label: "Personal Care", value: "personal-care", count: categoryCounts["personal-care"] ?? 0 },
                { label: "Household", value: "household", count: categoryCounts["household"] ?? 0 },
                { label: "Baby Care", value: "baby-care", count: categoryCounts["baby-care"] ?? 0 },
            ],
        },
        {
            title: "Dietary",
    ...
    const priceMax = Number(searchParams.get("maxPrice") || "22");
    ...
    const applyPriceFilter = useCallback(() => {
        pushParams((params) => {
            if (localPrice >= 22) params.delete("maxPrice");
    ...
    return (
        <aside className={cn("w-[280px] shrink-0", className)}>
            <div className="sticky top-[132px] max-h-[calc(100vh-160px)] overflow-y-auto pr-2 scrollbar-hide rounded-2xl border border-border/50 bg-background/70 backdrop-blur-sm p-5 shadow-soft">
                <h3 className="text-lg font-bold mb-4 bg-gradient-to-r from-primary to-emerald-500 bg-clip-text text-transparent">Filters</h3>
                ...
                <CollapsibleSection title="Price Range" defaultOpen>
                    <div className="mt-4 px-1">
                        ...
                        <div className="flex justify-between text-xs text-muted-foreground mt-2">
                            <span>$0</span>
                            <span className="font-medium text-foreground">${localPrice >= 22 ? "22+" : localPrice}</span>
                        </div>
                    </div>
                </CollapsibleSection>
                                ...
                                {group.options.map((opt) => (
                                    <div key={opt.value} className="flex items-center gap-2">
                                        <Checkbox
                                            ...
                                            onCheckedChange={() => toggleValue(group.paramKey, opt.value)}
