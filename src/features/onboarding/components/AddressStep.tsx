...
interface OnboardingAddressStepProps {
    ...
    onUpdateAddress: (field: keyof OnboardingState["address"], value: string) => void;
}

export function OnboardingAddressStep({ state, onUpdate, onUpdateAddress }: OnboardingAddressStepProps) {
    const [showDropdown, setShowDropdown] = useState(false);
    const [locating, setLocating] = useState(false);
    const [locationError, setLocationError] = useState<string | null>(null);

    const selectedCountry = COUNTRY_CODES.find(c => c.code === state.countryCode) || COUNTRY_CODES[0];
    ...
    const reverseGeocode = async (lat: number, lon: number) => {
        ...
        if (data.address) {
            ...
            onUpdateAddress("city", addr.city || addr.town || addr.village || addr.county || "");
    ...
    const handleUseCurrentLocation = async () => {
        ...
        const getBrowserPosition = (highAccuracy: boolean, timeoutMs: number): Promise<GeolocationPosition> => {
        ...
        try {
            ...
            if (err instanceof Error && "code" in err && err.code === 1) {
                setLocationError("Location access denied. Please enable location in Windows Settings → Privacy → Location, then allow your browser access.");
            } else {
    ...
    return (
        <div className="space-y-6 animate-in slide-in-from-right-4">
            <div>
                ...
                <p className="text-muted-foreground font-body">Where should we deliver your fresh groceries?</p>
            </div>
                                ...
                                <div className="absolute top-full left-0 mt-1 z-50 w-64 max-h-60 overflow-y-auto rounded-md border border-border bg-card shadow-lg">
                                    {COUNTRY_CODES.map((country) => (
                                        <button
                                            ...
                                            className={`w-full flex items-center gap-3 px-3 py-2.5 text-sm hover:bg-accent transition-colors text-left ${state.countryCode === country.code ? "bg-primary/10 font-medium" : ""
                                                }`}
                                        >
