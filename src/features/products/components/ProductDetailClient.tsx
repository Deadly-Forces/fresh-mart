...
const MOCK_REVIEWS = [
    { reviewerName: "Sarah M.", rating: 5, date: "Feb 18, 2026", comment: "Always fresh and great quality. We order regularly and are never disappointed." },
    { reviewerName: "James K.", rating: 4, date: "Feb 12, 2026", comment: "Good product! Arrived quickly and well-packaged. Would recommend." },
    { reviewerName: "Priyanka S.", rating: 5, date: "Feb 5, 2026", comment: "Excellent quality. Noticeably better than what I usually find at the supermarket." },
];
...
export function ProductDetailClient({ product }: { product: any }) {
    ...
    const currentVariant = product.variants?.find((v: any) => v.id === selectedVariant) || { price: product.price, name: 'Regular' };
    ...
    return (
        <div className="container mx-auto px-4 max-w-7xl py-6">
            ...
            <nav className="flex items-center gap-1.5 text-sm text-muted-foreground mb-8">
                ...
                <Link href={`/category/${product.categorySlug}`} className="hover:text-primary transition-colors">{product.categoryLabel}</Link>
                <ChevronRight className="w-3.5 h-3.5" />
            ...
            <div className="flex flex-col lg:flex-row gap-10 mb-16">
                ...
                <div className="w-full lg:w-[52%]">
                    ...
                    <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-2 px-1">
                        {product.images.map((img: string, i: number) => (
                            <button
                                key={i}
                                className={`w-20 h-20 shrink-0 rounded-button border-2 overflow-hidden transition-colors ${selectedImage === i ? "border-primary" : "border-border hover:border-primary/50"
                                    }`}
                ...
                <div className="w-full lg:w-[48%] lg:sticky lg:top-[120px] lg:self-start pl-0 lg:pl-6">
                    ...
                    {/* Price */}
                    <div className="mb-6">
                        <PriceDisplay price={currentVariant.price} comparePrice={product.comparePrice} size="lg" />
                    </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {product.variants.map((v: any) => (
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-pill text-sm font-medium border-2 transition-all ${selectedVariant === v.id
                    ...
                    <div className="bg-gradient-to-br from-secondary/40 to-background rounded-2xl p-5 space-y-4 border border-border/50 shadow-sm relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-3.5 text-sm relative z-10">
                            ...
                            <div>
                                <p className="font-bold text-[13px] text-foreground tracking-tight">Free Delivery</p>
                                <p className="text-muted-foreground text-[12px] font-medium mt-0.5">Estimated delivery: Tomorrow, 9 AM – 12 PM</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3.5 text-sm relative z-10">
                            <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center shrink-0 text-primary">
                            ...
                            <div>
                                <p className="font-bold text-[13px] text-foreground tracking-tight">Freshness Guarantee</p>
                                <p className="text-muted-foreground text-[12px] font-medium mt-0.5">Full refund if not satisfied with freshness</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-6">
                        {product.tags.map((tag: string) => (
                            <span key={tag} className="bg-primary-muted text-primary text-xs font-medium px-3 py-1 rounded-pill capitalize">
                                {tag}
                            </span>
                        ))}
                    </div>
            ...
            <Tabs defaultValue="description" className="mb-16 max-w-full overflow-hidden">
                <TabsList className="bg-secondary/50 h-14 md:h-12 p-1.5 rounded-card mb-8 w-full flex justify-start overflow-x-auto scrollbar-hide snap-x snap-mandatory">
                    <TabsTrigger value="description" className="data-[state=active]:bg-card rounded-button h-full px-4 md:px-6 text-[13px] md:text-sm font-medium shrink-0 snap-start">Description</TabsTrigger>
                    <TabsTrigger value="nutrition" className="data-[state=active]:bg-card rounded-button h-full px-4 md:px-6 text-[13px] md:text-sm font-medium shrink-0 snap-start">Nutritional Info</TabsTrigger>
                    <TabsTrigger value="reviews" className="data-[state=active]:bg-card rounded-button h-full px-4 md:px-6 text-[13px] md:text-sm font-medium shrink-0 snap-start">Reviews ({product.reviewsCount})</TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="reviews" id="reviews">
                    ...
                    <div className="flex flex-col md:flex-row gap-8 mb-8">
                        <div className="text-center md:text-left">
                            <p className="text-5xl font-bold text-foreground mb-1">{product.rating ?? 0}</p>
                            <div className="flex gap-0.5 mb-1 justify-center md:justify-start">
                                {Array.from({ length: 5 }).map((_, i) => (
                                    <Star key={i} className={`w-4 h-4 ${i < Math.floor(product.rating ?? 0) ? "fill-warning text-warning" : "fill-muted text-muted"}`} />
                                ))}
                            </div>
                            <p className="text-sm text-muted-foreground">{product.reviewsCount ?? 0} reviews</p>
                        </div>
                            ...
                            {[5, 4, 3, 2, 1].map((star) => (
                                <div key={star} className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                                        <div className="h-full bg-warning rounded-full" ref={(el) => { if (el) el.style.width = `${[65, 25, 8, 2, 0][5 - star]}%`; }} />
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            <section>
                <h2 className="font-heading text-2xl md:text-3xl mb-6">You May Also Like</h2>
                <div className="flex gap-4 md:gap-6 overflow-x-auto pb-4 scrollbar-hide snap-x">
                    {Array.from({ length: 5 }).map((_, i) => (
                        <div key={i} className="min-w-[160px] md:min-w-[220px] aspect-[3/4] bg-card rounded-card border border-border flex items-center justify-center text-muted-foreground text-sm shrink-0 snap-start">
                            Product {i + 1}
                        </div>
                    ))}
                </div>
