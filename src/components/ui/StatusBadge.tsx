...
const statusConfig: Record<string, { label: string; bg: string; text: string }> = {
    ...
    processing: { label: "Processing", bg: "bg-warning/10", text: "text-warning" },
    confirmed: { label: "Confirmed", bg: "bg-blue-500/10", text: "text-blue-600" },
    packed: { label: "Packed", bg: "bg-blue-500/10", text: "text-blue-600" },
    out_for_delivery: { label: "Out for Delivery", bg: "bg-purple-500/10", text: "text-purple-600" },
...
export function StatusBadge({ status, className }: StatusBadgeProps) {
    const config = statusConfig[status] || { label: status || "Unknown", bg: "bg-secondary", text: "text-muted-foreground" };
